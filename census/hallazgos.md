...
### 44. Ninguna pieza transparente de armaduras y armas cuelga de un `BSOrderedNode`
**ORIGEN**: PR [#88](https://github.com/FacundoSu1986/FacundoSu1986-skyrim-3d-pipeline/pull/88).

**AFIRMACIÓN**: de las **262** piezas `BSLightingShaderProperty` con `NiAlphaProperty` de *blending* (bit 0) en los NIF de armaduras y armas, **234 cuelgan de la raíz, 28 de un `NiNode` y 0 de un `BSOrderedNode`**. El alfa sale de la textura en **203** (sin `VERTEX_ALPHA`) y de los colores de vértice en **59** (con `VERTEX_ALPHA` y un alfa que varía). En **58 de esas 59** el RGB de los colores de vértice es blanco (ningún canal baja de 0,99); la otra es la flecha de Auriel (`dlc01/weapons/auriel/aurielsarrow.nif`, `Arrow:1`), con el RGB de 0 a 1. `BSOrderedNode` aparece en **244** de los 22.394 NIF del corpus, casi todos efectos (`effects/greybeardwords` 22, `dlc02/effects` 17, `dlc01/effects` 10) y gemas (`clutter/gemstones` 13); en armaduras y armas, en uno solo —`dlc01/armor/dwarven/1stpersondwarvenshieldcrystal.nif`—, y ese nodo **no tiene hijos**.
**CONSULTA QUE LA PRODUJO**: con PyNifly 27.2 dentro de Blender 4.4.1, cada NIF de `armor/`, `weapons/`, `dlc0*/armor/` y `dlc0*/weapons/`; de cada pieza con `NiAlphaProperty` de flags con el bit 0 y shader `BSLightingShaderProperty`, el `blockname` de su padre, el bit `VERTEX_ALPHA` de `Shader_Flags_1` y si el alfa de sus colores de vértice varía más de 0,01; de las que tienen `VERTEX_ALPHA`, el mínimo y el máximo de los tres canales de color. Los 244 archivos con `BSOrderedNode`, por el nombre del tipo en la cabecera; los hijos del de Dawnguard, con `nif_nodos.leer`, que lee los bytes por su cuenta.
**N**: 2.148 NIF (ninguno que PyNifly no pudiera abrir), 262 piezas con *blending*; 22.394 NIF en la búsqueda del tipo.
**EXCEPCIONES ENCONTRADAS**: ninguna pieza con *blending* bajo un `BSOrderedNode`. Las piezas `BSEffectShaderProperty` (brillos, sangre) no se contaron: no son la receta de un vidrio. Por eso `exportar_nif.py` cuelga las piezas transparentes de la raíz y no escribe el parche de la trampa 12 de `asset-nuevo-skyrim`, que el escudo ovalado usó sin que molestara.
