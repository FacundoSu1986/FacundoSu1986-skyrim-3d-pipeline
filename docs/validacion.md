...
| `verificar_plugin.py <MiMod.esl>` | 7 REGLAS: `formVersion` 44 en cada record; índice de mod ≤ cantidad de masters; WEAP con DATA de 10 bytes y DNAM de 100; el `Prn` del NIF según el tipo de animación; referencia colocada en grupo 8 con `0x400` y en grupo 9 sin; `OFST` de WRLD dentro del archivo; `FULL` de texto en un plugin no localizado | 10.273 de 10.273 records autorados para SE; 1.188.810 de 1.188.811; 3.359 de 3.359 WEAP; 305 de 306 armas del jugador; 879.753 referencias sin excepción; 76.250 de 76.250 entradas de OFST; 34.956 FULL localizadas de 4 bytes | `test_verificar_plugin.py`, `test_prn_arma.py` y el autotest | sí |
...
| `exportar_nif.py -- <plan.json> <asset.blend>` y `exportar_nif.py -- --falsificar` (Blender) | el NIF de un asset nuevo con la estructura del donante; antes de reemplazar el definitivo lo relee con PyNifly y con `nif_nodos.py` (piezas, triángulos, shader, texturas, la V, `Prn`, los bloques de colisión) y le pasa las REGLAS de `colision_caja.py`. Copia la `NiAlphaProperty` de la receta; con alfa por vértice exige la capa `VERTEX_ALPHA` y que no quede pareja en 1 (opaca: trampa 16) ni en 0 (invisible); pareja en otro valor, y un alfa pintado que la receta no usa, dejan una nota. Rechaza donantes sin inercia. `--falsificar`: un donante sintético y cuatro planes: el bueno (metal, un vidrio con alfa por vértice, otro con el alfa parejo en 0,5 y un panel con el alfa en la textura) y tres que tienen que reprobar | las de `colision_caja.py`; la inercia es heurística declarada | la parte pura: `test_exportar_puro.py` y el autotest de `exportar_puro.py`, con diez mutantes que lo ponen rojo (seis del alfa); con `BLENDER_EXE`, `test_exportar_nif_blender.py`, con tres | la parte pura |
...
| `verificar_plugin.py --falsificar <Data>` | sobre plugins reales: los que pasan tal cual son el patrón, y torcer cada campo tiene que reprobar. Los masters de 2011 quedan afuera a propósito (no pasan la REGLA 1). Las REGLAS 5-7 se juzgan solas sobre los 10: referencias de grupo 8 sin `0x400` y de grupo 9 con, OFST fuera del archivo, FULL localizada en un plugin no localizado | vacío |
...
6. La suite entera: `python -m unittest discover -s tests`.
...
skills, de `fixtures/` y de `tests/`, y el juego.
