...
| `exportar_nif.py -- <plan.json> <asset.blend>` y `exportar_nif.py -- --falsificar` (Blender) | el NIF de un asset nuevo con la estructura del donante; antes de reemplazar el definitivo lo relee con PyNifly y con `nif_nodos.py` (piezas, triángulos, shader, texturas, la V, `Prn`, los bloques de colisión) y le pasa las REGLAS de `colision_caja.py`. Copia la `NiAlphaProperty` de la receta; con alfa por vértice exige la capa `VERTEX_ALPHA` y que varíe (trampa 16). Rechaza donantes sin inercia. `--falsificar`: un donante sintético y cuatro planes: el bueno (metal, un vidrio con alfa por vértice y un panel con el alfa en la textura) y tres que tienen que reprobar | las de `colision_caja.py`; la inercia es heurística declarada | la parte pura: `test_exportar_puro.py` y el autotest de `exportar_puro.py`, con cuatro mutantes que lo ponen rojo; con `BLENDER_EXE`, `test_exportar_nif_blender.py` | la parte pura |
