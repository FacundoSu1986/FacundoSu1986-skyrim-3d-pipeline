...
| `preparar_parte.py --planar <grados> [--soldadura F]` (Blender) | no reprueba: avisa si el decimado quedó más de un 15 % arriba del presupuesto, y dice si la soldadura tiene que ser más chica o más grande. Lo que reprueba es `salud_malla.py` sobre sus `.obj`. `--planar G` funde los paneles planos antes de colapsar (disuelve: no mueve vértices), y si el resultado queda en n-gonos triangula antes de guardar | `--planar` es un parámetro de la preparación, como `--soldadura`: no comprueba nada. **El ángulo no está medido** y la evidencia es el render; el pase en sí está `[sin verificar]` (no se corrió con Blender, y el enum `DISSOLVE` se corrigió contra la API, no contra una corrida) | — | no |
