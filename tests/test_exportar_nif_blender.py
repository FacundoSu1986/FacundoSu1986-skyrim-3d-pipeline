...
cuatro planes --el bueno, con metal, un vidrio con alfa por vertice, otro con
el alfa parejo en 0,5 y un panel con el alfa en la textura, sale con las
recetas copiadas y conciliadas, el alfa como estaba en la escena y sus notas;
un vidrio sin capa VERTEX_ALPHA, uno con la capa pareja en 1 y un donante sin
inercia reprueban--. Tres mutantes la ponen roja: el RGB negro sin capa base
(lo que hace el exportador de PyNifly) y las dos notas del alfa que no llegan
al informe.
"""
import os
...
class ExportarNifEnBlenderTests(unittest.TestCase):
    ...
    def test_falsificar(self):
        ...
        self.assertIn("falsificar: 20 comprobaciones, 0 fallas", p.stdout)
