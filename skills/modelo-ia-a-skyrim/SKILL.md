...
- **`scripts/salud_malla.py`** — mide si la malla se **rompió** al decimarla, y
  ...
  **se cuentan, no se declaran**) y `--falsificar <carpeta>`, que rompe mallas
  vanilla de hasta cuatro formas por archivo —el número depende de si hay
  triángulos interiores— y exige que el control las pesque. Sale con `2` si
  los argumentos no sirven (otra extensión), para que un automatizador
